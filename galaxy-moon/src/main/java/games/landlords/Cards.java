...
public class Cards {
    ...
    public void put(CardModule cardModule, Integer number){
        if(number < 0 || number > 4){
            throw new IllegalArgumentException("number of "+cardModule.getName()+" is "+number);
        }
    ...
    public static Cards newFullCards() {
        Cards newCards = new Cards();
        for (CardModule card : CardModule.values()) {
            if(card.getValue() < CardModule.CARD_BLACK_KING.getValue()){
                newCards.put(card, 4);
            }
        }
    ...
    public static Cards newCards(String s){
        if(!s.matches("([2-9]|0|J|Q|K|A|B|R)*")){
            throw new IllegalArgumentException("illegal cards: "+s);
        }
